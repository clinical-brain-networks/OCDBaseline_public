...
from nilearn.image import new_img_like, resample_to_img, coord_transform
...
roi_labels_dict = {'Savage_threat': {'rois': [0, 1, 2],
                                     'labels': ['rightInsula',
                                                'acc',
                                                'leftInsula']},
                   'Savage_safety': {'rois': [2, 3],
                                     'labels': ['vmpfc',
                                                'pcc']}}
...
for spm in ['Savage_threat', 'Savage_safety']:
    ...
    for i, roi_i in enumerate(roi_labels_dict[spm]['rois']):
        ...
        # save roi
        nib.save(roi_img, roi_dir+spm+'_'+roi_labels[i]+'.nii.gz')
...
df_coords.to_csv(roi_dir+'Savage_coords.csv', index=False)
