# %%
'''
Perform statistics and plots on connectivity results
...
def save_nbs(pval, adj, thresh, perms, contrast, test, ts_method):
    ...
    for comp in range(len(pval)):
        ...
        out = ('nbs_mats/nbs_thresh-'+str(thresh)
               + '_perms-'+str(perms)
                + '_contrast-'+contrast
                + '_test-'+test
                + '_comp_n-'+str(comp)
...
#  1 sample t-test (paired t-test in NBS)
for thresh in nbs_thresh:

        # Threat 0 > 1
        try:
            pval, adj, null = nbs_bct(con_mats_sym[:, :, 0, :],
                                        con_mats_sym[:, :, 1, :],
                                        thresh, k=perms,
                                        tail='both', paired=True,
                                        seed=seed)
            df = save_nbs(pval, adj, thresh, perms,
                            'threat', '1samp', ts_method)
        ...
        # Safety 2 > 3
        # a try statement is used because the effect is so
        # weak in this contrast that NBS throws an error
        try:
            pval, adj, null = nbs_bct(con_mats_sym[:, :, 2, :],
                                    con_mats_sym[:, :, 3, :],
                                        thresh, k=perms,
                                        tail='both', paired=True,
                                        seed=seed)
            df = save_nbs(pval, adj, thresh, perms,
                            'safety', '1samp', ts_method)
            nbs_df = pd.concat([nbs_df, df])
        except:
            print("Oops!", sys.exc_info(), "occurred.")
            print('Carrying on...')
...
for thresh in nbs_thresh:
        try:
            ...
            x = con_data[:, :, group_idx == 'control']
            y = con_data[:, :, group_idx == 'patient']
...
# save out
if eroded:
    nbs_df.to_csv(result_dir+'nbs_results_perms-'+str(perms)+ \
